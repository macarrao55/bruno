    ...
    switch ($module) {
        ...
        case 'custo_funcionario_clt':
            ...
            if ($action === 'employee_cost_add') {
                ...
                $totalMonthlyCost = $baseSalary + $inssCommon + $fgts + $thirteenthProvision + $vacationProvision + $extraExpense1 + $extraExpense2 + $gasCommission;
...
$cardReportFilters = [
    'date_from' => trim((string) ($_GET['report_card_date_from'] ?? date('Y-m-01'))),
    'date_to' => trim((string) ($_GET['report_card_date_to'] ?? $today)),
    'machine' => trim((string) ($_GET['report_card_machine'] ?? '')),
    'brand' => trim((string) ($_GET['report_card_brand'] ?? '')),
    'card_type' => trim((string) ($_GET['report_card_type'] ?? '')),
];
$cardReportSql = 'SELECT * FROM card_receivables WHERE canceled=0';
$cardReportWhere = [];
$cardReportParams = [];
if ($cardReportFilters['date_from'] !== '') {
    $cardReportWhere[] = 'sale_date >= :report_date_from';
    $cardReportParams[':report_date_from'] = $cardReportFilters['date_from'];
}
if ($cardReportFilters['date_to'] !== '') {
    $cardReportWhere[] = 'sale_date <= :report_date_to';
    $cardReportParams[':report_date_to'] = $cardReportFilters['date_to'];
}
if ($cardReportFilters['machine'] !== '') {
    $cardReportWhere[] = 'machine = :report_machine';
    $cardReportParams[':report_machine'] = $cardReportFilters['machine'];
}
if ($cardReportFilters['brand'] !== '') {
    $cardReportWhere[] = 'brand = :report_brand';
    $cardReportParams[':report_brand'] = $cardReportFilters['brand'];
}
if ($cardReportFilters['card_type'] !== '') {
    $cardReportWhere[] = 'card_type = :report_card_type';
    $cardReportParams[':report_card_type'] = normalizeCardType($cardReportFilters['card_type']);
}
if ($cardReportWhere !== []) {
    $cardReportSql .= ' AND ' . implode(' AND ', $cardReportWhere);
}
$cardReportSql .= ' ORDER BY sale_date DESC, id DESC';
$cardReportRows = fetchAll($pdo, $cardReportSql, $cardReportParams);
$cardReportSummary = [
    'quantity' => count($cardReportRows),
    'gross_total' => array_reduce($cardReportRows, static fn(float $sum, array $row): float => $sum + (float) ($row['gross_value'] ?? 0), 0.0),
    'net_total' => array_reduce($cardReportRows, static fn(float $sum, array $row): float => $sum + (float) ($row['net_value'] ?? 0), 0.0),
    'fee_total' => array_reduce($cardReportRows, static fn(float $sum, array $row): float => $sum + ((float) ($row['gross_value'] ?? 0) - (float) ($row['net_value'] ?? 0) + (float) ($row['anticipation_discount'] ?? 0)), 0.0),
];
$cardReportByMachine = fetchAll($pdo, 'SELECT machine, COUNT(*) AS quantity, COALESCE(SUM(gross_value),0) AS gross_total, COALESCE(SUM(net_value),0) AS net_total, COALESCE(SUM(gross_value - net_value + COALESCE(anticipation_discount,0)),0) AS fee_total
    FROM card_receivables
    WHERE canceled=0 AND sale_date BETWEEN :start AND :end
        AND (:machine = "" OR machine = :machine)
        AND (:brand = "" OR brand = :brand)
        AND (:card_type = "" OR card_type = :card_type)
    GROUP BY machine
    ORDER BY gross_total DESC', [
    ':start' => $cardReportFilters['date_from'],
    ':end' => $cardReportFilters['date_to'],
    ':machine' => $cardReportFilters['machine'],
    ':brand' => $cardReportFilters['brand'],
    ':card_type' => $cardReportFilters['card_type'] !== '' ? normalizeCardType($cardReportFilters['card_type']) : '',
]);
$cardReportByDay = fetchAll($pdo, 'SELECT sale_date, COALESCE(SUM(gross_value),0) AS gross_total, COALESCE(SUM(net_value),0) AS net_total
    FROM card_receivables
    WHERE canceled=0 AND sale_date BETWEEN :start AND :end
        AND (:machine = "" OR machine = :machine)
        AND (:brand = "" OR brand = :brand)
...
<?php elseif ($module === 'custo_funcionario_clt'): ?>
    ...
    <p>Este módulo calcula o custo mensal real do funcionário incluindo salário, encargos, provisões, despesas extras e comissão de gás.</p>
    <p class="small">A comissão de gás é registrada no custo mensal, mas não entra no bloco de Despesas com Pessoal do DRE.</p>
    <form method="post" id="employeeCostCreateForm">
        ...
        <input type="number" step="0.01" min="0" name="inss_rate" value="11" placeholder="% INSS comum">
        ...
        <input type="number" step="0.01" min="0" name="extra_expense_1" placeholder="Outras despesas 1">
    ...
    <table>
        <tr><th>Mês</th><th>Funcionário</th><th>Salários</th><th>INSS comum</th><th>FGTS</th><th>Provisão 13º</th><th>Provisão Férias</th><th>Outras despesas 1</th><th>Outras despesas 2</th><th>Comissão gás</th><th>Total mensal</th><th>Ações</th></tr>
        <?php foreach ($employeeMonthlyCosts as $cost): ?>
            <tr>
                ...
                <td><?= money((float) $cost['inss_common']) ?></td>
                <td><?= money((float) $cost['fgts']) ?></td>
                <td><?= money((float) $cost['thirteenth_provision']) ?></td>
                <td><?= money((float) $cost['vacation_provision']) ?></td>
                <td><?= money((float) ($cost['extra_expense_1'] ?? 0)) ?></td>
                <td><?= money((float) ($cost['extra_expense_2'] ?? 0)) ?></td>
...
<?php elseif ($module === 'relatorio_cartoes'): ?>
    <h3>Relatório de Cartões</h3>
    <form method="get">
        <input type="hidden" name="module" value="relatorio_cartoes">
        <label>Data inicial <input type="date" name="report_card_date_from" value="<?= htmlspecialchars($cardReportFilters['date_from']) ?>"></label>
        <label>Data final <input type="date" name="report_card_date_to" value="<?= htmlspecialchars($cardReportFilters['date_to']) ?>"></label>
        <label>Maquininha
            <select name="report_card_machine">
                <option value="">Todas</option>
                <?php foreach ($cardMachines as $machine): ?>
                    <option value="<?= htmlspecialchars((string) $machine['name']) ?>" <?= $cardReportFilters['machine'] === (string) $machine['name'] ? 'selected' : '' ?>><?= htmlspecialchars((string) $machine['name']) ?></option>
                <?php endforeach; ?>
            </select>
        </label>
        <label>Bandeira
            <select name="report_card_brand">
                <option value="">Todas</option>
                <?php foreach ($cardBrands as $brand): ?>
                    <option value="<?= htmlspecialchars((string) $brand['name']) ?>" <?= $cardReportFilters['brand'] === (string) $brand['name'] ? 'selected' : '' ?>><?= htmlspecialchars((string) $brand['name']) ?></option>
                <?php endforeach; ?>
            </select>
        </label>
        <label>Forma de pagamento
            <select name="report_card_type">
                <option value="">Todas</option>
                <option value="debito" <?= $cardReportFilters['card_type'] === 'debito' ? 'selected' : '' ?>>Débito</option>
                <option value="credito_avista" <?= $cardReportFilters['card_type'] === 'credito_avista' ? 'selected' : '' ?>>Crédito à vista</option>
                <option value="credito_parcelado" <?= $cardReportFilters['card_type'] === 'credito_parcelado' ? 'selected' : '' ?>>Crédito parcelado</option>
            </select>
        </label>
        <button type="submit">Aplicar</button>
        <a href="?module=relatorio_cartoes">Limpar</a>
    </form>

    <div class="cards">
        <div class="card"><h4>Quantidade</h4><p><?= (int) $cardReportSummary['quantity'] ?></p></div>
        <div class="card"><h4>Valor bruto</h4><p><?= money((float) $cardReportSummary['gross_total']) ?></p></div>
        <div class="card"><h4>Valor líquido</h4><p><?= money((float) $cardReportSummary['net_total']) ?></p></div>
    ...
    <table>
        ...
        <?php foreach ($cardReportByMachine as $row): ?>
            <tr>
    ...
    <h4>Lançamentos filtrados</h4>
    <table>
        <tr><th>Data venda</th><th>Maquininha</th><th>Bandeira</th><th>Tipo</th><th>Bruto</th><th>Líquido</th><th>Taxas</th><th>Recebido</th></tr>
        <?php foreach ($cardReportRows as $row): ?>
            <tr>
                <td><?= dateBr((string) $row['sale_date']) ?></td>
                <td><?= htmlspecialchars((string) $row['machine']) ?></td>
                <td><?= htmlspecialchars((string) $row['brand']) ?></td>
                <td><?= htmlspecialchars((string) (($row['card_type_original'] ?? '') !== '' ? $row['card_type_original'] : $row['card_type'])) ?></td>
                <td><?= money((float) $row['gross_value']) ?></td>
                <td><?= money((float) $row['net_value']) ?></td>
                <td><?= money((float) ((float) $row['gross_value'] - (float) $row['net_value'] + (float) ($row['anticipation_discount'] ?? 0))) ?></td>
                <td><?= (int) $row['received'] === 1 ? 'Sim' : 'Não' ?></td>
            </tr>
        <?php endforeach; ?>
    </table>
    <script>
        new Chart(document.getElementById('cardReportDayChart'), {
            type: 'line',
            data: {
                labels: <?= json_encode(array_map(static fn(array $row): string => dateBr((string) $row['sale_date']), $cardReportByDay)) ?>,
                datasets: [
                    { label: 'Bruto', data: <?= json_encode(array_map(static fn(array $row): float => (float) $row['gross_total'], $cardReportByDay)) ?>, borderColor: '#2563eb' },
                    { label: 'Líquido', data: <?= json_encode(array_map(static fn(array $row): float => (float) $row['net_total'], $cardReportByDay)) ?>, borderColor: '#16a34a' }
                ]
            }
        });
        new Chart(document.getElementById('cardReportMachineChart'), {
            type: 'bar',
            data: {
                labels: <?= json_encode(array_map(static fn(array $row): string => (string) $row['machine'], $cardReportByMachine)) ?>,
                datasets: [
                    { label: 'Bruto', data: <?= json_encode(array_map(static fn(array $row): float => (float) $row['gross_total'], $cardReportByMachine)) ?>, backgroundColor: '#2563eb' },
                    { label: 'Taxas', data: <?= json_encode(array_map(static fn(array $row): float => (float) $row['fee_total'], $cardReportByMachine)) ?>, backgroundColor: '#f97316' }
                ]
            }
        });
    </script>
...
<?php elseif ($module === 'dre'): ?>
    ...
    <table id="dreReportTable">
        ...
        <tr><td>&nbsp;&nbsp;INSS comum</td><td><?= money($dre['despesas_pessoal_inss_comum']) ?></td></tr>
        ...
        <tr><td>&nbsp;&nbsp;Outras despesas 1</td><td><?= money($dre['despesas_pessoal_extra_1']) ?></td></tr>
        <tr><td>&nbsp;&nbsp;Outras despesas 2</td><td><?= money($dre['despesas_pessoal_extra_2']) ?></td></tr>
